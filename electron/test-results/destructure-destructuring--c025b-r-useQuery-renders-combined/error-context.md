...
- Name: destructure.spec.ts >> destructuring variable declarations >> const { data, error } = useQuery() renders combined
- Location: test/destructure.spec.ts:56:3
...
- Define function App
+
+     FilesApp2.tsxApp2.tsx1import { useQuery } from 'react';import {useQuery} from react2function App() {App. No parameters3  const { data, error } = useQuery();data and error = the result of calling useQuery4  return null;return `null`5}6 
+     
+   
+
+
...
- button "Collapse sidebar":
  - img
- button "App2.tsx"
- heading "App2.tsx" [level=3]
- table:
  - rowgroup:
    - 'row "1 import { useQuery } from ''react''; import {useQuery} from react"':
      - cell
      - cell "1"
      - 'cell "import { useQuery } from ''react'';"'
      - cell
      - 'cell "import {useQuery} from react"'
    ...
    - 'row "3 const { data, error } = useQuery(); data and error = the result of calling useQuery"':
      - cell
      - cell "3"
      - 'cell "const { data, error } = useQuery();"'
      - cell
      - cell "data and error = the result of calling useQuery"
